...
#include "detours.h"
//#include "syelog.h"
#define SyelogV DbgPrintFile
#define Syelog DbgPrintFile

#define SYELOG_SEVERITY_NOTICE 0
#define SYELOG_SEVERITY_INFORMATION 1
#define SYELOG_SEVERITY_FATAL 2
#define SYELOG_FACILITY_APPLICATION 1
...
void DbgPrintFile(const int LEVEL, const CHAR* Format, ...);
...
VOID APIENTRY DetourFinishHelperProcess()
{

}

WCHAR szLogFileName[256] = { 0 };
...
void DbgPrintFile(const int LEVEL, const CHAR* Format, ...)
{
	...
	if (szLogFileName[0] == 0)
		StringCchPrintfW(szLogFileName, 256, L"c:\\users\\public\\SPI_log_%d.log", GetCurrentProcessId());

	va_list vargs;
	va_start(vargs, Format);
	StringCchVPrintfA(szOutputText, 2048, Format, vargs);
	va_end(vargs);

	GetSystemTime(&SystemTime);
	_hFile = Real_CreateFileW(szLogFileName,
	...
	if (_hFile != INVALID_HANDLE_VALUE)
	{
		...
		{
			StringCchPrintfA(szTimeStamp, 64, "%02i/%02i %02i:%02i:%02i.%03i [%05i] - ",
				SystemTime.wMonth,
				SystemTime.wDay,
				SystemTime.wHour,
				SystemTime.wMinute,
				...
				GetCurrentThreadId());
			size_t len;
			StringCchLengthA(szTimeStamp, 64, &len);
			WriteFile(_hFile, szTimeStamp, (DWORD)len, &dwBytesWritten, NULL);

			if (szOutputText[0] != 0)
			{
				StringCchLengthA(szOutputText, 2048, &len);

				if (szOutputText[len - 1] == '\n')
					szOutputText[len - 1] = 0;
			}

			WriteFile(_hFile, szOutputText, (DWORD)len, &dwBytesWritten, NULL);
